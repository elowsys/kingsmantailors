...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190131210225_apiUserRoleAmend')
BEGIN
    ALTER TABLE [datRole] ADD [roleAbbrev] nvarchar(max) NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20190131210225_apiUserRoleAmend', N'2.2.1-servicing-10028');
END;

GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20190131225155_apiOccasionLabelAmend')
BEGIN
    DECLARE @var0 sysname;
    SELECT @var0 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[infOccasionLabel]') AND [c].[name] = N'labelName');
    IF @var0 IS NOT NULL EXEC(N'ALTER TABLE [infOccasionLabel] DROP CONSTRAINT [' + @var0 + '];');
    ALTER TABLE [infOccasionLabel] ALTER COLUMN [labelName] nvarchar(max) NULL;
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20190131225155_apiOccasionLabelAmend', N'2.2.1-servicing-10028');
END;
